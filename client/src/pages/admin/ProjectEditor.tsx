...
export function ProjectEditor() {
  ...
  if (loading) return <div className={adminStyles.page}><p className="text-white/40 section-container">Loading...</p></div>;
    ...
    <div className={adminStyles.page}>
      <div className={adminStyles.container}>
        <div className={adminStyles.header}>
          ...
          <h1 className={adminStyles.title}>{isNew ? "New Project" : "Edit Project"}</h1>
          <div />
        </div>
        ...
        <form className={adminStyles.form} onSubmit={handleSubmit}>
          <div className={adminStyles.field}>
            ...
            <input className={adminStyles.input} value={title} onChange={(e) => setTitle(e.target.value)} required />
          </div>
          <div className={adminStyles.field}>
            <label className={adminStyles.label}>Description</label>
            <textarea className={`${adminStyles.input} ${adminStyles.textarea}`} value={description} onChange={(e) => setDescription(e.target.value)} required />
          </div>
          <div className={adminStyles.field}>
            <label className={adminStyles.label}>Tech Stack (comma separated)</label>
            <input className={adminStyles.input} value={techStack} onChange={(e) => setTechStack(e.target.value)} placeholder="React, TypeScript, Node.js" />
          </div>
          <div className={adminStyles.field}>
            <label className={adminStyles.label}>Demo URL</label>
            <input className={adminStyles.input} value={demoUrl} onChange={(e) => setDemoUrl(e.target.value)} type="url" />
          </div>
          <div className={adminStyles.field}>
            <label className={adminStyles.label}>GitHub URL</label>
            <input className={adminStyles.input} value={githubUrl} onChange={(e) => setGithubUrl(e.target.value)} type="url" />
          </div>

          <div className={adminStyles.row}>
            <label className={adminStyles.checkbox}>
          ...
          <div className={adminStyles.field}>
            <label className={adminStyles.label}>Images</label>
            <label>
            ...
            <div className={adminStyles.previews}>
              {images.map((img) => (
                <img key={img.fileId} src={getMediaUrl(img.fileId)} alt="" className={adminStyles.preview} />
              ))}
            </div>
          </div>

          <div className={adminStyles.field}>
            <label className={adminStyles.label}>Video</label>
            <label>
            ...
            {video && <p className="text-white/40 text-sm mt-2">{video.filename}</p>}
          </div>
