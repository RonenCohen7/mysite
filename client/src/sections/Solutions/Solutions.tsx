    ...
    <Section id="solutions">
      ...
      <div className={solutionsStyles.grid}>
        {t.solutions.map((solution, si) => (
          <motion.div key={solution.title} ref={(el) => { refs.current[si] = el; }} className={solutionsStyles.card} whileHover={{ y: -4 }}>
            ...
            <div className={solutionsStyles.steps}>
              {solution.steps.map((step, i) => (
                <div key={step} className={solutionsStyles.step}>
                  <div className={`${solutionsStyles.stepBox} ${i === 0 ? solutionsStyles.stepActive : ""}`}>{step}</div>
                  {i < solution.steps.length - 1 && (
                    <div className={solutionsStyles.arrow}><div className={solutionsStyles.line} /><ArrowDown size={16} /></div>
                  )}
                </div>
              ))}
            </div>
