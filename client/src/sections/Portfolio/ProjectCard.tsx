  ...
  return (
    <GlassCard className={portfolioStyles.card} hover>
      <div className={portfolioStyles.imageWrap}>
        {coverUrl ? (
          <img src={coverUrl} alt={project.title} className={portfolioStyles.image} loading="lazy" />
        ) : (
          <div className={portfolioStyles.placeholder}>
            <span className="text-4xl font-bold text-gradient">{project.title.charAt(0)}</span>
          </div>
        )}
      </div>
