import type { TechIconSource } from "@models/techStack";
import "./TechIcon.css";
...
function CustomTechIcon({ id }: { id: "cursor" | "deepseek" | "gpt" }) {
  if (id === "cursor") {
    return (
      <svg viewBox="0 0 24 24" className={"tech-icon__svg"} aria-hidden="true">
...
export function TechIcon({ icon, name }: TechIconProps) {
  ...
  return (
    <span className={"tech-icon__fallback-wrap"}>
      <img
        src={src}
        alt=""
      ...
      />
      <span data-fallback className={"tech-icon__fallback"} style={{ display: "none" }}>
        {name.slice(0, 2)}
      </span>
    </span>
  );
